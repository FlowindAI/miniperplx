...
import { convertToCoreMessages, streamText, tool, smoothStream } from "ai"
    ...
    tools: {
                    ...
                    if (imageData && typeof imageData === "string") {
                      ...
                      try {
                        ...
                        if (error instanceof BlobRequestAbortedError) {
                          ...
                          console.error(
                            "Error saving image to Vercel Blob:",
                            error
                          )
                        }
                      }
      ...
      text_translate: tool({
        ...
        parameters: z.object({
          ...
          to: z
            .string()
            .describe("The language to translate to (e.g., 'fr' for French)."),
          from: z
            .string()
            .describe(
              "The source language (optional, will be auto-detected if not provided)."
            ),
        }),
        execute: async ({
          text,
      ...
      nearby_search: tool({
        ...
        parameters: z.object({
          ...
          radius: z
            .number()
            .default(6000)
            .describe("The radius in meters (max 50000, default 6000)."),
        }),
        execute: async ({
          location,
          latitude,
          ...
          try {
            ...
            if (geocoding.results?.[0]?.geometry?.location) {
              let trimmedLat = geocoding.results[0].geometry.location.lat
                ...
                .split(".")
              finalLat = parseFloat(
                trimmedLat[0] + "." + trimmedLat[1].slice(0, 6)
              )
              let trimmedLng = geocoding.results[0].geometry.location.lng
                .toString()
                .split(".")
              finalLng = parseFloat(
                trimmedLng[0] + "." + trimmedLng[1].slice(0, 6)
              )
              ...
              nearbyData.data.map(async (place: any) => {
                try {
                  if (!place.location_id) {
                    console.log(
                      `Skipping place "${place.name}": No location_id`
                    )
                    return null
                  }
                  ...
                  try {
                    ...
                    console.log(
                      `Photo fetch failed for "${place.name}":`,
                      error
                    )
                  }
                  ...
                  return {
                    ...
                    location: {
                      lat: parseFloat(
                        details.latitude || place.latitude || finalLat
                      ),
                      lng: parseFloat(
